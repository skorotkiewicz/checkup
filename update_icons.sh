...
#
# This script reads Phosphor SVG source files and produces Rust constants
# with width="16" height="16" and fill="currentColor" attributes.
...
# ── Helper: transform a raw Phosphor SVG into an inline-ready string ─────
transform_svg() {
    local file="$1"
    local content
    content="$(cat "$file")"

    # 1. Add width="16" height="16" to the <svg> tag
    content="$(echo "$content" | sed 's/<svg /<svg width="16" height="16" /')"
...
HEADER
    ...
    for name in "${ORDERED[@]}"; do
        ...
        echo "/// $doc"
        echo "pub const ${name}: &str = r#\"${svg_content}\"#;"
        echo
    done
...
echo "Done! Updated $(grep -c 'pub const' "$OUTPUT") icon constants in $OUTPUT"
